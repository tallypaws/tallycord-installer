...
func main() {
	...
	if *versionFlag {
		fmt.Println("Tallycord Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2023 Vendicated and Tallycord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Tallycord Installer' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Tallycord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Tallycord will likely not work.")
}
