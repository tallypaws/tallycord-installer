...
func main() {
	...
	win = g.NewMasterWindow("Tallycord Installer", 1200, 800, 0)
...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest Tallycord builds from GitHub:\n"+err.Error())
	}
	return
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		g.Style().SetFontSize(20).To(
			renderErrorCard(
				...
				"**Github** and **vencord.dev** are the only official places to get Vencord. Any other site claiming to be us is malicious.\n"+
					"tallycord also isnt evil (honestly idk what to put here)\n"+
				...
				g.Style().
					...
					To(
						g.Button("Reinstall / Repair").
							...
							Size((w-40)/4, 50),
						Tooltip("Reinstall & Update Tallycord"),
					),
				g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify Tallycord installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise Tallycord will likely not work.\n\n"+
		...
		RawInfoModal("#openasar-confirm", "OpenAsar", "OpenAsar is an open-source alternative of Discord desktop's app.asar.\n"+
			"Tallycord is in no way affiliated with OpenAsar.\n"+
...
func loop() {
	...
	g.SingleWindow().
		...
		Layout(
			...
			g.Style().SetFontSize(20).To(
				g.Row(
					g.Label(Ternary(IsDevInstall, "Dev Install: ", "Tallycord will be downloaded to: ")+TallycordDirectory),
					g.Style().
						SetColor(g.StyleColorButton, DiscordBlue).
						SetStyle(g.StyleVarFramePadding, 4, 4).
						To(
							g.Button("Open Directory").OnClick(func() {
								g.OpenURL("file://" + path.Dir(TallycordDirectory))
							}),
						),
				),
				...
				g.Label("Local Tallycord Version: "+InstalledHash),
				&CondWidget{
					GithubError == nil,
					func() g.Widget {
						if IsDevInstall {
							return g.Label("Not updating Tallycord due to being in DevMode")
						}
						return g.Label("Latest Tallycord Version: " + LatestHash)
					}, func() g.Widget {
